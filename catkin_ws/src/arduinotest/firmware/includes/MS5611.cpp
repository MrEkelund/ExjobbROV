...
constructor
*/
AP_Baro_MS5611::AP_Baro_MS5611(int chipSelect):_MS5611ChipSelect(chipSelect){}

void MS5611::_init()
{
  pinMode(_MS5611ChipSelect, OUTPUT);

  SPIWrite(CMD_MS5611_RESET)
  _dev->transfer(&CMD_MS5611_RESET, 1, nullptr, 0);
  delay(4);
...
void MS5611::SPIWrite(byte thisCommand) {
  // take the chip select low to select the device:
  digitalWrite(_MS5611ChipSelect, LOW);

  SPI.transfer(thisCommand);  //Send command

  // take the chip select high to de-select:
  digitalWrite(_MS5611ChipSelect, HIGH);
}

void MS5611::SPIWriteRegister(byte thisRegister, byte thisValue) {
...
uint32_t AP_Baro_MS56XX::_read_adc()
{
  ...
  if (!_dev->transfer(&CMD_MS5611_READ_ADC, 1, val, 3)) {
    return 0;
  }
