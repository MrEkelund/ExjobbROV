...
// degrees -> radians
static inline float radians2(float deg) {
	return deg * DEG_TO_RAD;
}

// radians -> degrees
static inline float degrees2(float rad) {
	return rad * RAD_TO_DEG;
}

// square
static inline float sq2(float v) {
	return v*v;
}

// 2D vector length
static inline float pythagorous2(float a, float b) {
	return sqrtf(sq2(a)+sq2(b));
}

// 3D vector length
static inline float pythagorous3(float a, float b, float c) {
	return sqrtf(sq2(a)+sq2(b)+sq2(c));
}
