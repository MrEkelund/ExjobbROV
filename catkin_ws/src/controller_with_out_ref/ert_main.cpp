...
static ControllerModelClass Controller_with_out_ref_Obj;// Instance of model class 
...
{
  runModel = (rtmGetErrorStatus(Controller_with_out_ref_Obj.getRTM()) == (NULL));
  while (runModel) {
    sem_wait(&baserateTaskSem);
    Controller_with_out_ref_Obj.step();
...
{
  rtmSetErrorStatus(Controller_with_out_ref_Obj.getRTM(), "stopping the model");
}
...
void terminateTask(void *arg)
{
  ...
  // Terminate model
  Controller_with_out_ref_Obj.terminate();
  sem_post(&stopSem);
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(Controller_with_out_ref_Obj.getRTM(), 0);

  // Initialize model
  Controller_with_out_ref_Obj.initialize();
