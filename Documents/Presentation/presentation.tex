...
\item Syftet med ett reglersystem
\end{itemize}}
\end{frame}
...
\begin{itemize}
\item \structure<1> {BlueROV paket från Blue Robotics}
\item \structure<2> {Raspberry Pi 2}
\item \structure<3> {Raspicam}
\item \structure<4> {HkPilot 2.7}
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
\only<1-4>{
...
\begin{column}{0.5\textwidth}
\begin{itemize}
\item \structure<1> {Vad är en modell och hur kan den användas?}
\item \structure<2> {Notation}
\item \structure<3-4> {Koordinatsystem}
\end{itemize}
\end{column}
...
\begin{figure}
    ...
    \end{tikzpicture}
\end{figure}
\note{Hur de två olika koordinatsystemen är relaterade. Samt hur vinklar är definierade.}
\end{frame}
...
%\end{subequations}
\note{\begin{itemize}
\item Identifierbarhet är om man kan urskilja vilken parameter som påverkar vad
\end{itemize}}
\end{frame}
...
\begin{column}{0.5\textwidth}
\begin{itemize}
\item \structure<1-2> {Vad är parameter skattning?}
\item \structure<3> {Kontrollsignaler}
\item \structure<4> {Insignaler och utsignaler}
\item \structure<5> {Validering av modellen}
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
...
\begin{frame}{Prediktionsfels metoden}
\begin{columns}
	\begin{column}{0.5\textwidth}
	\begin{itemize}
	\item \structure<1> {Skattningsstruktur}
	\item \structure<2-3> {Kostnadsfunktion}
	\end{itemize}
	\end{column}
...
\begin{columns}
	\begin{column}{0.5\textwidth}
	\begin{itemize}
	\item \structure<1> {Estimation structure}
	\end{itemize}
	\end{column}
...
\begin{columns}
	\begin{column}{0.5\textwidth}
	\begin{align*}
	\dot{\hat{\eulerAngles}} &= \boldsymbol{T}_\theta(\hat{\eulerAngles})\angVelBody \\ &\quad\Longleftrightarrow\quad \\
	\hat{\eulerAngles} &= \int \boldsymbol{T}_\theta(\hat{\eulerAngles})\angVelBody
	\end{align*}
	\end{column}
...
\begin{columns}
	...
	\begin{itemize}
	\item \structure<1> {Estimation structure}
	\item \structure<2> {Initial state estimation}
	\end{itemize}
	\end{column}
...
\begin{columns}
	\begin{column}{0.5\textwidth}
	\begin{itemize}
	\item \structure<2> {Utöka tillståndsvektorn}
	\item \structure<3> {Använd den kompletta modellen vid tidsuppdateringen}
	\end{itemize}
	\end{column}
...
\begin{columns}
	\begin{column}{0.5\textwidth}
	\structure<2> {Dålig anpassning i P}
	\structure<3-> {Truster 6}
	\end{column}
	
	\begin{column}{0.5\textwidth}
	\only<3-3>{\includegraphics[width=\textwidth]{fig/thruster6u6}}
	\only<4-4>{\includegraphics[width=\textwidth]{fig/thruster6p}}
	\only<5-5>{\includegraphics[width=\textwidth]{fig/thruster6r}}
	\end{column}
\end{columns}
\note{
\begin{itemize}
...
\begin{column}{0.5\textwidth}
\begin{itemize}
\item  \structure<1> {Vad är reglerteknik}
\item  \structure<2>{Öppen styrning}
\item  \structure<3>{Styrning med återkoppling}
\item  \structure<4->{Exakt linjärisering}
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
\only<2-2>{
