...
\note{\begin{itemize}
\item The BlueROV package from Blue Robotics comes with 6 thrusters, 6 ESCs and an acrylic tube.
...
\begin{column}{0.5\textwidth}
\begin{itemize}
\item \structure<1-2> {What is parameter estimation?}
\item \structure<3> {Input signals}
\item \structure<4> {Validation of a model}
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
...
\end{columns}
\note{\begin{itemize}
\item Parameter estimation is to estimate parameters in a model. The model can either be known (Gray-box) or unknown (Black-box).
\item If parameters are not well estimated could problems arise when synthesising model-based controllers.
\end{itemize}}
\end{frame}

\begin{frame}{Prediction-Error Method}
\begin{columns}
	\begin{column}{0.5\textwidth}
	\begin{itemize}
	\item \structure<1> {Estimation structure}
	\item \structure<2-3> {Cost function}
	\end{itemize}
	\end{column}
...
\end{columns}
\note{\begin{itemize}
\item Angles from sensor fusion and thruster control signals as inputs.
\item Angular velocities as output.
\end{itemize}}
\end{frame}
...
\note{
\begin{itemize}
\item En sak vi la märke till var låg fit i p
