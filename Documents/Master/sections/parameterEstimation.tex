...
Using the model defined in \sectionref{sec:estimation_angular} with the estimated angles as inputs and angular velocities as outputs gave the estimated parameters that can be seen in \Tableref{tab:ResultEstimAngular}. The esimation used two datasets as estimation data and used one dataset for validation data. The fit of the model compared to validation data can be seen in \Figureref{fig:velocityCompareCong}
...
\begin{table}[hbp]
  ...
  \begin{tabular}{l l p{0.7\linewidth}}
	...
	$z_B$               & 	\meter 						&  \meter\\
    $\Kp$               &    	\kilogram\usk\meter\squared 	&   	\kilogram\usk\meter\squared\\
    $\Kpabsp$           &   	\kilogram\usk\meter\squared	& 	  		\kilogram\usk\meter\squared\\
    $\Mq$               &   	\kilogram\usk\meter\squared	&   \kilogram\usk\meter\squared\\
    $\Mqabsq$           &   	\kilogram\usk\meter\squared	&   \kilogram\usk\meter\squared\\
    $\Nr$               &   	\kilogram\usk\meter\squared	&  	\kilogram\usk\meter\squared\\
    $\Nrabsr$           &   	\kilogram\usk\meter\squared	& 	\kilogram\usk\meter\squared\\
    $A_p$               &  	\kilogram\usk\meter\squared	&   	\kilogram\usk\meter\squared\\
    $B_q$               &  	\kilogram\usk\meter\squared	&  	\kilogram\usk\meter\squared\\
    $C_r$               &  	\kilogram\usk\meter\squared	&  		\kilogram\usk\meter\squared\\
    \bottomrule%
  \end{tabular}
\end{table}
...
Using the model defined in \sectionref{sec:angLinEstim} with angular velocities and linear acceleration as outputs gave the estimated the parameter values as can be seen in \tableref{tab:ResultangVellz60} and \Tableref{tab:ResultangVellz6}. The estimation was done with four datasets and two datasets was used for validation.
...
\end{align}
The extremely low values of the last five elements in $\boldsymbol{Q}$ were chosen to fix the measured moment arms of the thrusters. Four data sets were feed in to the estimator and a fifth was used as validation data. All data sets were of excitations in $p$, $q$ and $r$. Parameter values form the estimation can be vied in \tableref{tab:ResultKalmanFixedMomentArms}.
\begin{table}[hbp]
  \centering
