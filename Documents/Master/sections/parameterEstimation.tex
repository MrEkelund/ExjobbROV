...
\Warning[TODO]{Write about kinetics in \Chapterref{cha:modelling}}
...
% Lägg de här någon lämplig plats
...
Due to poorly estimated angles as can be seen in \Figureref{fig:integratedAngleVelocities}, the angles was not suited to be used in the parameter estimation. Thus was the parameters only estimated using the angular velocities as measurements. This method was used when the sensor fusion algorithms described in \Sectionref{sec:angularVelocityKalman} was used.
...
\begin{figure}[tbp]
  ...
  \caption{\label{fig:integratedAngleVelocities}%
    As can be seen in \protect\subref{fig:velocityAnglePhi}-\protect\subref{fig:velocityAnglePsi} the angles are poorly estimated and thus not suitable to use in parameter estimation.}
\end{figure}
...
\begin{figure}[tbp]
  \centering
  \subfloat[][\label{fig:velocityAnglePhi}Comparison between a simulated model without reparametrisation and validation data in the \rollAngle-direction.]{\includegraphics[width=0.5\textwidth]{velocityCompareCongp}}
  \qquad
  \subfloat[][\label{fig:velocityAngleTheta}Comparison between a simulated model without reparametrisation and validation data in the \pitchAngle-direction.]{\includegraphics[width=0.5\textwidth]{velocityCompareCongq}}
  \\
  \subfloat[][\label{fig:velocityAnglePsi}Comparison between a simulated model without reparametrisation and validation data in the \yawAngle-direction.]{\includegraphics[width=0.5\textwidth]{velocityCompareCongr}}
  \caption{\label{fig:integratedAngleVelocities}%
    Comparison of simulation of the attitude model against validation data.}
\end{figure}