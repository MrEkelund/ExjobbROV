...
\end{equation}
and the predictor can be described by \todo[inline]{Detta ar inte fixat. Men just nu star det samma sak som i Roger Larssons tes}
\begin{equation}
...
To handle noisy signals an error threshold was introduced. An error threshold means that after a breakpoint the cost function becomes linear instead of quadratic, which makes the parameter estimation less sensitive to outliers. To reduce the impact of the noise even further, the used weight matrix in the quadratic cost function was chosen as the inverse of the estimated noise covariance. The used linear cost function was
\begin{equation}
   V(\boldsymbol{\theta}) = \frac{1}{N} \sum_{t=1}^{N} \abs{\boldsymbol{e}^T_k(\boldsymbol{\theta})} \boldsymbol{W}_{\text{lin}}(\boldsymbol{\theta})
\end{equation}
where $\boldsymbol{W}_{\text{lin}}$ is the weight vector with positive elements in the linear cost function. 
