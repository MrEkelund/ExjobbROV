...
\begin{equation}
\Inertia_{\text{A}} \dot{\nuVector} +\boldsymbol{C}_{\text{A}}(\nuvector)\nuVector + \boldsymbol{D}(\nuVector) = \tau_{Hydro} 
\end{equation} 
where $\boldsymbol{C}_{\text{A}}(\nuvector)\nuVector$ and $\Inertia_{\text{A}}$ models the Coriolis forces and inertia from the added mass of the water respectively. The vector $\boldsymbol{D}(\nuVector)$ models the waters linear and quadratic damping effects.
