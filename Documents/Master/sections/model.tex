...
\end{split}
\end{equation}
where \distance{x}{1}, \distance{x}{2}, \distance{y}{1}, \distance{y}{2}, \distance{y}{3}, \distance{y}{4} and \distance{z}{6} are the offsets in the $x$, $y$ or $z$ direction of the $i$:th thruster and $\thrusterfun{i}$ is a lookup table from control signal to thrust in Newtons. See \Appref{app:thrustmapping} for details regarding the lookup table.
...
Changing from Euler angles to quaternions also changes \eqref{eq:restoring_forces} to 
\begin{equation}
...
The kinematic equation \eqref{eq:kineticsEuler}  is changed to \index{Kinematics}
\begin{equation}
